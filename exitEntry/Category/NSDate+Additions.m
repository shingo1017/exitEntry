...
- (NSString *)onlineDateFormattedString {
    ...
    if (minDiffInterval < 1) {
        ...
        formattedString = [NSString stringWithFormat:@"%i-%i", components.month, components.day];
    }
...
- (NSString *)longDateFormattedString {
    ...
    NSDate *now = [[NSDate date] UMTDate];
    ...
    if (minDiffInterval < 1) {
        //小于1分钟
        formattedString = text(@"刚刚");
    }
    else if (hourDiffInterval < 1) {
        //小于1小时
        formattedString = [NSString stringWithFormat:@"%i %@", minDiffInterval, text(@"分钟前")];
    }
    else if (hourDiffInterval < 24) {
        
        //小于1天
        formattedString = [NSString stringWithFormat:@"%i %@", hourDiffInterval, text(@"小时前")];
    }
    else if (secDiffIntervalSinceToday > 0 && secDiffIntervalSinceToday < 3600*24) {
        
        //昨天
        formattedString = text(@"昨天");
    }
    else {
...
- (NSString *)vipValidityDateFormattedString {
    ...
    if (secDiffIntervalSinceToday > 0) 
        formattedString = @"您目前还未有VIP特权";
    else 
        formattedString = [NSString stringWithFormat:@"%i年%i月%i日", components.year, components.month, components.day];
